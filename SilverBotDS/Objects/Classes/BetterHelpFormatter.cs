        ...
        public override BaseHelpFormatter WithSubcommands(IEnumerable<Command> subcommands)
        {
            if(Command == null)
            {
                ...
                foreach (var command in subcommands)
                {
                    ...
                    if (command.Module.ModuleType.GetCustomAttributes(true).Any(x => x.GetType() == typeof(CategoryAttribute)))
                    {
                        foreach (var attribute in command.Module.ModuleType.GetCustomAttributes(true).Where(x => x is CategoryAttribute))
                        {
                            foreach (var category in ((CategoryAttribute)attribute).Category)
                            {
                                if (!commands.ContainsKey(category))
