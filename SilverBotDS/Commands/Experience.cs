...
namespace SilverBotDS.Commands
{
    ...
    public class Experience : BaseCommandModule, IRequireAssets
    {
        ...
        public async Task XpCard(CommandContext ctx, DiscordUser user)
        {
            ...
            var picturetext = Image.Text(HttpUtility.HtmlEncode($"{user.Username}#{user.Discriminator}"),
                "Twemoji Color Emoji, Diavlo Light", 596,
                42, Enums.Align.Low,
                true, fontfile: "fonts/twemoji.otf");
            ...
            if (o is not null)
            {
                ...
                using var nextLvlText = Image.Text(HttpUtility.HtmlEncode($"{GetNeededXpForNextLevel(o.XP)}XP"),
                    "Diavlo Light", 229,
                    22, Enums.Align.Low,
                    true);
