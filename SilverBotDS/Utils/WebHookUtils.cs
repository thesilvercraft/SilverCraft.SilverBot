...
internal class WebHookUtils
{
    ...
    public static void ParseWebhookUrlNullable(string webhookUrl, out ulong? webhookIdnullable, out string webhookToken)
    {
        try
        {
            ParseWebhookUrl(webhookUrl, out var webhookId, out webhookToken);
    ...
    public static void ParseWebhookUrl(string webhookUrl, out ulong webhookId, out string webhookToken)
    {
        ...
        if (match != null)
        {
            ...
            if (!(match.Groups[2].Success && ulong.TryParse(match.Groups[2].Value, NumberStyles.None,
                    CultureInfo.InvariantCulture, out webhookId))) throw ex("The webhook Id could not be parsed.");

            if (!match.Groups[3].Success) throw ex("The webhook token could not be parsed.");
