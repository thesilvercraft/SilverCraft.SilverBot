...
public class NeutralAudio 
{
    ...
    {
        if (ctx is BasedCommandContext bsdctx)
        {
            ...
            {
                await Play(ctx,
                    (SongORSongs)await ctx.CommandsNext.ConvertArgument(bsdctx.Message.ReferencedMessage.Attachments[0].Url,
    ...
    public async Task Loop(ISilverBotContext ctx, [Description("Has to be none, song or queue")] LoopSettings settings)
    {
        var lang = await LanguageService.FromCtxAsync(ctx);
        await MakeSureBothAreInVC(ctx, lang);
        var player = AudioService.GetPlayer<BetterVoteLavalinkPlayer>(ctx.Guild.Id);
    ...
    public async Task VoteSkip(ISilverBotContext ctx)
    {
        ...
        if (ctx is BasedCommandContext bcc &&  await new RequireDjAttribute().ExecuteCheckAsync(bcc, false))
        {
             await ctx.SendMessageAsync(  lang.CanForceSkip, language: lang);
        }

        if (ctx is UnBasedCommandContext ubcc && await new RequireDjSlashAttribute().ExecuteChecksAsync(ubcc))
        {
            await ctx.SendMessageAsync( lang.CanForceSkip, language: lang);
