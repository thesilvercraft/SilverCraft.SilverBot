...
namespace SilverBotDS.Commands.Slash
{
    public class GeneralCommands : ApplicationCommandModule
    {
            ...
            await ctx.CreateResponseAsync(InteractionResponseType.ChannelMessageWithSource,
                new DiscordInteractionResponseBuilder().AddEmbed(new DiscordEmbedBuilder()
                    ...
                    .AddField(lang.JoinedSilverCraft,
                        StringUtils.BoolToEmoteString(
                            await Genericcommands.IsAtSilverCraftAsync(ctx.Client, user, Cnf)),
                        true)
                    ...
                    .AddField(lang.AccountCreationDate,
                        user.CreationTimestamp.UtcDateTime.DateTimeToTimeStamp(TimestampFormat.LongDateTime), true)
                    .AddField(lang.AccountJoinDate,
                        (ctx.Guild?.Members.ContainsKey(user.Id) == true
                            ? ctx.Guild?.Members[user.Id].JoinedAt.UtcDateTime
        ...
        public async Task VersionInfoCommand(InteractionContext ctx)
        {
            ...
            await ctx.CreateResponseAsync(InteractionResponseType.ChannelMessageWithSource,
                new DiscordInteractionResponseBuilder().AddEmbed(MiscCommands.VersionInfoEmbed(lang, ctx))
                    .AsEphemeral(true));
        }
