    ...
    public sealed class Genericcommands : BaseCommandModule
    {
        ...
        public async Task Time(CommandContext ctx)
        {
            ...
            await new DiscordMessageBuilder()
                ...
                .WithContent(string.Format(lang.TimeInUtc,
                    DateTime.UtcNow.DateTimeToTimeStamp(TimestampFormat.LongDateTime)))
                .SendAsync(ctx.Channel);
        }
        ...
        {
            var msg = await new DiscordMessageBuilder().WithReply(ctx.Message.Id)
                .WithContent($"🏓 Pong! {ctx.Client.Ping}ms")
                .SendAsync(ctx.Channel);
            await msg.ModifyAsync(
                $"🏓 Pong! {ctx.Client.Ping.Milliseconds().Humanize()} ({(msg.Timestamp - ctx.Message.Timestamp).Humanize()} to send, {(DateTime.UtcNow - ctx.Message.Timestamp).Humanize()} round trip)");
        }
        ...
        public async Task ArchiveMessage(CommandContext ctx, DiscordMessage message)
        {
            ...
            using (ZipArchive zip = new(memoryStream, ZipArchiveMode.Create, true))
            {
                ...
                await using MemoryStream zipItemMsgFileStream = new(Encoding.UTF8.GetBytes(message.Content))
                    { Position = 0 };
        ...
        public async Task Userinfo(CommandContext ctx,
            [Description("the user (left blank = person running command)")] DiscordUser a)
        {
            var lang = await LanguageService.FromCtxAsync(ctx);
            await new DiscordMessageBuilder().WithEmbed(new DiscordEmbedBuilder()
                ...
                .AddField(lang.AccountCreationDate,
                    a.CreationTimestamp.UtcDateTime.DateTimeToTimeStamp(TimestampFormat.LongDateTime), true)
                .AddField(lang.AccountJoinDate,
                    ctx.Guild?.Members.ContainsKey(a.Id) == true
                        ? ctx.Guild?.Members[a.Id].JoinedAt.UtcDateTime
                            .DateTimeToTimeStamp(TimestampFormat.LongDateTime)
                        : "NA", true)
