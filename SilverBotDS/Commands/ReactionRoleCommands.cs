...
namespace SilverBotDS.Commands
{
    ...
    public partial class ReactionRoleCommands : BaseCommandModule, IHaveExecutableRequirements
    {
        ...
        public async Task ReactionRoleAdd(CommandContext ctx)
        {
            ...
            if (!(ctx.Guild.Permissions?.HasPermission(Permissions.ManageRoles) == true ||
                  ctx.Guild.CurrentMember.Roles.MaxBy(x => x.Position)!.CheckPermission(Permissions.ManageRoles) ==
            ...
            bool GetFromContent(string content, bool def = false)
            {
                ...
                if (lang.ReactionRoleResponseYes == ctl || lang.ReactionRoleResponseYes2 == ctl ||
                    lang.ReactionRoleResponseYes3 == ctl)
                {
                    return true;
                }

                if (lang.ReactionRoleResponseNo == ctl || lang.ReactionRoleResponseNo2 == ctl ||
                    lang.ReactionRoleResponseNo3 == ctl)
                {
                    return false;
                }
            ...
            var result = await ctx.Message.GetNextMessageAsync(m =>
            {
                ...
                return lang.ReactionRoleResponseYes == ctl || lang.ReactionRoleResponseYes2 == ctl ||
                       lang.ReactionRoleResponseYes3 == ctl || lang.ReactionRoleResponseNo == ctl ||
                       lang.ReactionRoleResponseNo2 == ctl || lang.ReactionRoleResponseNo3 == ctl;
            });
            ...
            if (!result.TimedOut)
            {
                ...
                while (true)
                {
                    ...
                    try
                    {
                        foreach (var line in lines)
                        {
                            ...
                            var role = ulong.TryParse(splitLine[0], out var id)
                                ? ctx.Guild.GetRole(id)
                                : ctx.Guild.Roles.FirstOrDefault(r => r.Value.Mention == splitLine[0]).Value;
                            var m = Emote().Match(splitLine[1]);
                            var emoji = m.Success
                                ? DiscordEmoji.FromGuildEmote(ctx.Client, ulong.Parse(m.Groups["id"].Value))
                                : DiscordEmoji.FromUnicode(ctx.Client, splitLine[1]);
                            var t = ReactionRoleType.Normal;
                            if (splitLine.Length == 3)
                            {
                                if (ushort.TryParse(splitLine[2], out var idOfType) && Enum
                                        .GetValues<ReactionRoleType>().Select(x => (ushort)x).Contains(idOfType))
                                {
                            ...
                            roles.Add(role, new Tuple<DiscordEmoji, ReactionRoleType>(emoji, t));
                        }
                    }
                    catch (Exception e)
                    {
                    ...
                    await result.Result.DeleteAsync();
                    StringBuilder rolesAlreadyAddedMsgContent =
                        new($"{lang.ReactionRoleRolesAdded} ({roles.Count}):\n");
                    foreach (var role in roles)
                    {
                        rolesAlreadyAddedMsgContent.Append(role.Value.Item1).Append(' ').Append(role.Key.Name)
                            .Append(' ').AppendLine(role.Value.Item2.Humanize());
                    }
                ...
                if (useembed)
                {
                    ...
                    mb = mb.WithEmbed(new DiscordEmbedBuilder().WithTitle(title)
                        .WithDescription(roleMenuBodyText.ToString())
                        .WithColor(colour));
                }
                else
                {
                ...
                foreach (var role in roles)
                {
                    serverSettings.ReactionRoleMappings.Add(new ReactionRoleMapping
                    {
                        Emoji = role.Value.Item1.Name, EmojiId = role.Value.Item1.Id, RoleId = role.Key.Id,
                        MessageId = menuMessage.Id, ChannelId = ctx.Channel.Id, Mode = role.Value.Item2
                    });
