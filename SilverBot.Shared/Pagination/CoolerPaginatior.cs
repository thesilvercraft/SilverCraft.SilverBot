...
namespace SilverBot.Shared.Pagination
{
    public class CoolerPaginatior : IPaginator
    {
                    ...
                    if (Guid.TryParse(args.Id[4..], out var guid))
                    {
                        async Task DoLogic(bool next)
                        {
                            if (Paginations.TryGetValue(guid, out var pagination))
                            {
                                ...
                                {
                                    var nextPage =
                                        pagination.Pagination.GetPageAtId(
                                            next ? (int)page.NextId : (int)page.PreviousId);
                                    await args.Interaction.CreateResponseAsync(
                                        InteractionResponseType.UpdateMessage,
                                        new DiscordInteractionResponseBuilder()
                                            .WithContent(nextPage.Content.HasValue ? nextPage.Content.Value : null)
                                            .AddEmbed(nextPage.Embed.HasValue ? nextPage.Embed.Value : null)
                                            .AddComponents(GetButtons(nextPage, pagination.Pagination, guid)));
        ...
        {
            if (page.PreviousId != null && page.PreviousId < pagination.AllowedRange.End.Value &&
                page.PreviousId >= pagination.AllowedRange.Start.Value)
            {
                yield return new DiscordButtonComponent(ButtonStyle.Primary, "CIPB" + id.ToString("N"), null, false,
                    new DiscordComponentEmoji("◀️"));
            }

            if (page.NextId != null && page.NextId < pagination.AllowedRange.End.Value &&
                page.NextId >= pagination.AllowedRange.Start.Value)
            {
        ...
        public async Task<DiscordMessage> SendPage(ILazyPage page, DiscordChannel channel, IPagination pagination,
            Guid id)
        {
