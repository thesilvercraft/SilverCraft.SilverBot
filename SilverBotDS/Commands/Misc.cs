...
namespace SilverBotDS.Commands
{
    ...
    public class MiscCommands : BaseCommandModule
    {
        ...
        public async Task VersionInfoCmd(CommandContext ctx)
        {
            ...
            await new DiscordMessageBuilder().WithEmbed(VersionInfoEmbed(lang, ctx)).WithReply(ctx.Message.Id)
        ...
        public async Task SetLanguage(CommandContext ctx, string langName)
        {
                ...
                .Any(x => string.Equals(x, langName, StringComparison.InvariantCultureIgnoreCase)))
            {
                ...
                {
                    Database.UpsertUserLanguageCode(ctx.User.Id, langName.ToLowerInvariant());
                }
                else
                {
                    Database.UpsertGuildLanguageCode(ctx.Guild.Id, langName.ToLowerInvariant());
                }
            ...
            else
            {
                await new DiscordMessageBuilder().WithReply(ctx.Message.Id)
                    .WithContent(
                        $"Unknown language. Valid choices are:```{LanguageService.LoadedLanguages().Humanize()}```")
                    .SendAsync(ctx.Channel);
            }
        }
        ...
        public async Task WhereIs(CommandContext ctx, [RemainingText] string commandname)
        {
            ...
            {
                y--;
                if (text[y] != 'n' || text[y - 1] != 'o' || text[y - 2] != 's' || text[y - 3] != 'j' ||
                    text[y - 4] != '`' || text[y - 5] != '`' || text[y - 6] != '`')
                {
                    continue;
                }
